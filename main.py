...
from Shared import *
from utils import is_moderator, fetch_user_objs, get_lineup_str, get_teams_string
    ...
    for id in list(events.keys()):
        ...
        if len(removed_unconfirmed_players) != 0:
            await event.channel.send(
                f"{', '.join(removed_unconfirmed_players)} has been removed from unconfirmed squads due to a mogi filling in <#{channel_id}>")

        if len(removed_players) != 0:
            await event.channel.send(
                f"{', '.join(removed_players)} has been removed from the lineup due to a mogi filling in <#{channel_id}>")
...
class Event:
    ...
    async def end_mogi(self, ctx):
        ...
        elif datetime.now() - self.fill_time > timedelta(minutes=40) and datetime.now() - self.fill_time < timedelta(
                minutes=60) and not ctx.author.name in self.queue_flat:
            return
    ...
    async def remove(self, channel, number, bypass):
        ...
        if not bypass:
            await channel.send(
                f"{', '.join([user.mention for user in users])} has been removed from the mogi. {str(len(self.queue_flat))} players remaining.")
